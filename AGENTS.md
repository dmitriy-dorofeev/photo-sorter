...
- **Видео-метаданные**: планируется через внешний `exiftool` (пока не реализовано)
...
│   │   └── scanner_test.go
│   ├── dateresolver/
│   │   ├── dateresolver.go        # Приоритет: EXIF → имя файла → mtime → unsorted
...
- Go 1.24+
- `exiftool` (опционально, рекомендуется для будущей поддержки видео)
...
3. **dateresolver** — для каждого файла: EXIF (JPEG) → парсинг имени (8 паттернов) → `mtime` (если включено `UseModTime`) → `unsorted/`.
...
- **Фикстуры** — реальные и сгенерированные файлы в `testdata/`:
  ...
  - `e2e/source/` — набор из 12 файлов, имитирующих разные сценарии (iPhone, Android, WhatsApp, скриншоты, дубликаты, Live Photos, файлы без даты).

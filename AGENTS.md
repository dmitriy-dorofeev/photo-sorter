...
3. **dateresolver** — для каждого файла: EXIF (JPEG) → видео-метаданные (exiftool) → парсинг имени (8 паттернов) → `mtime` (если включено `UseModTime`) → `unsorted/`. Источник даты (`DateSource`) прокидывается дальше в pipeline.
...
- **Обратная синхронизация метаданных** — при включённом `--write-exif` (или настройке в TUI) файлы, дата которых определена только по имени или `mtime`, получают запись `DateTimeOriginal` через `exiftool`. Integrity-check выполняется до модификации метаданных, поэтому хеши не ломаются.
...
- Инкрементальные запуски (SQLite/bolt state в целевой папке).

...
3. **dateresolver** — для каждого файла: EXIF (JPEG) → видео-метаданные (exiftool, если доступен) → парсинг имени (8 паттернов) → `mtime` (если включено `UseModTime`) → `unsorted/`. Источник даты (`DateSource`) прокидывается дальше в pipeline. Путь к `exiftool` передаётся через `runner.Config.ExifToolPath`.

...
### 2.1 `internal/copier/copier.go:218` — `Sync()` на каждый файл
...
### 2.2 `internal/scanner/scanner.go:48` — `nil` слайс без capacity
...
### 5.2 `tui/model.go:30` — Только один Source в TUI
...
### 5.3 `tui/settings.go:116` + `cmd/main.go:80` — Несогласованные дефолты
...
### 5.4 `tui/settings.go:102` — Вводящий в заблуждение help
...
### 5.5 `tui/preview.go:59-61` — Некорректная замена `rel` на `/`
...
### 6.1 `cmd/main.go:159-164` — `os.Stat` не проверяет, что это директория
...
### 6.2 `cmd/main.go:143` — Нет проверки `target` на доступность для записи
...
### 6.3 `cmd/main.go:186-189` — Потеря частичной статистики при ошибке

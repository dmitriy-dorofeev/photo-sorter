...
- Определяет дату съёмки:
  - **Фото:** EXIF (`DateTimeOriginal`)
  - **Видео:** метаданные через `exiftool` (`DateTimeOriginal` → `CreateDate` → `MediaCreateDate`)
  - **Fallback:** парсинг имени файла (8 паттернов) → `mtime` (опционально)

...
- **Безопасно работает** — только копирование, никогда не перемещает; dry-run по умолчанию
...
1. **Всегда начинайте с dry-run:**
   ```bash
   ./photo-sorter --source ~/Photos --target /Volumes/ExternalDisk --dry-run=true
   ```
